...
GameScreen::GameScreen(GameData* game_data) : game_data_(game_data) {
	...
	table_ = std::make_unique<Table>(game_data_->texture_manager.get()->Get("pool_table"), game_data_->WIDTH, game_data_->HEIGHT);

	game_data->texture_manager->Get("white_ball")->SaveToFile("save.png");
	...
	// init holes
	// game_hole_r / (actualr=70) = table.w / table_tex.w
	float hole_radius = 70 * table_rect.w / game_data_->texture_manager->Get("pool_table")->GetWidth();
	// bottom left
	holes_.push_back(std::make_unique<Hole>(hole_radius + table_rect.x, hole_radius + table_rect.y, 1.0f));
	// bottom middle
	holes_.push_back(std::make_unique<Hole>(table_rect.x + table_rect.w / 2.0f, hole_radius / 2.0f + table_rect.y, 1.0f));
	// bottom right
	holes_.push_back(std::make_unique<Hole>(table_rect.x + table_rect.w - hole_radius, hole_radius + table_rect.y, 1.0f));
	// top left
	holes_.push_back(std::make_unique<Hole>(hole_radius + table_rect.x, table_rect.y + table_rect.h - hole_radius, 1.0f));
	// top middle
	holes_.push_back(std::make_unique<Hole>(table_rect.x + table_rect.w / 2.0f, table_rect.y + table_rect.h - hole_radius / 2.0f, 1.0f));
	// top right
	holes_.push_back(std::make_unique<Hole>(table_rect.x + table_rect.w - hole_radius, table_rect.y + table_rect.h - hole_radius, 1.0f));
}
...
void GameScreen::Render(float dt) {
	...
	// render hud
	sprite_batch->Render(game_data_->WIDTH * 0.0225f, game_data_->HEIGHT * 0.2f, game_data_->WIDTH * 0.05f, game_data_->HEIGHT * 0.6f, glm::vec4(0.0f, 0.2f, 0.2f, 1.0f));
	sprite_batch->Render(game_data_->WIDTH * 0.0225f, game_data_->HEIGHT * 0.2f, game_data_->WIDTH * 0.05f, game_data_->HEIGHT * 0.6f * ball_->GetChargePercent(), glm::vec4(1.0f, 1.0f, 0.1f, 1.0f));
	// debug
