...
Viewport::Viewport(ViewportType viewport_type,
                   u32 initial_width,
                   u32 initial_height)
...
math::rect<u32> Viewport::on_resize(u32 new_width, u32 new_height) {
    switch (viewport_type) {
        case ViewportType::fit: {
            const auto fit_aspect_ratio = [](f32 src_width,
                                             f32 src_height,
                                             f32 max_width,
                                             f32 max_height) {
                const f32 ratio =
                    math::min(max_width / src_width, max_height / src_height);
            ...
            u32 margin_left =
                (u32)math::round((new_width - viewport_size.x) / 2.0f);
            u32 margin_bottom =
                (u32)math::round((new_height - viewport_size.y) / 2.0f);

            glViewport(
                margin_left, margin_bottom, current_width, current_height);
