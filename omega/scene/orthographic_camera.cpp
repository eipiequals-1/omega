...
OrthographicCamera::OrthographicCamera(f32 left, f32 right,
                                       f32 bottom, f32 top,
                                       f32 near, f32 far) :
    Camera::Camera() {
...
void OrthographicCamera::set_projection(f32 left, f32 right,
                                        f32 bottom, f32 top,
                                        f32 near, f32 far) {
...
glm::vec2 OrthographicCamera::project(const glm::vec2 &vec,
                                      const glm::vec2 &screen_res) const {
    // convert to world size
    glm::vec2 convert;
    convert.x = vec.x / screen_res.x * width;
    convert.y = vec.y / screen_res.y * height;
    convert += glm::vec2(position.x, position.y);
    return convert;
}
