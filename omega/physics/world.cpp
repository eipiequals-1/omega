...
uptr<BoxBody> World::add_box(const glm::rectf &rect, BodyType type, b2FixtureDef fixture_def, float gravity_scale, float angle) {
    BoxBody *body = new BoxBody(rect, type, fixture_def, gravity_scale, angle, world.get());
    return uptr<BoxBody>(body);
}

uptr<CircleBody> World::add_circle(const glm::vec2 &center, float radius, BodyType type, b2FixtureDef fixture_def, float gravity_scale) {
