...
static uint32_t TextureFromFile(const std::string &filepath, const std::string &directory) {
	uint32_t id = 0;
...
Model::Model(const std::string &filepath) {
	...
	if (scene == nullptr || scene->mFlags & AI_SCENE_FLAGS_INCOMPLETE || scene->mRootNode == nullptr) {
		libgl::Log("Error: Assimp: ", importer.GetErrorString());
	}
...
Mesh Model::ProcessMesh(aiMesh *mesh, const aiScene *scene) {
	...
	// diffuse
	std::vector<MeshTexture> diffuse_maps = LoadMaterialTextures(material, aiTextureType_DIFFUSE, "textureDiffuse");
	textures.insert(textures.end(), diffuse_maps.begin(), diffuse_maps.end());
	// specular
	std::vector<MeshTexture> specular_maps = LoadMaterialTextures(material, aiTextureType_SPECULAR, "textureDiffuse");
	textures.insert(textures.end(), specular_maps.begin(), specular_maps.end());
	// normal
	std::vector<MeshTexture> normal_maps = LoadMaterialTextures(material, aiTextureType_HEIGHT, "textureNormal");
	textures.insert(textures.end(), normal_maps.begin(), normal_maps.end());
	// height
	std::vector<MeshTexture> height_maps = LoadMaterialTextures(material, aiTextureType_AMBIENT, "textureHeight");
...
std::vector<MeshTexture> Model::LoadMaterialTextures(aiMaterial *mat, aiTextureType type, const std::string &type_name) {
	...
	for (uint32_t i = 0; i < mat->GetTextureCount(type); ++i) {
		...
		bool skip = false;
		for (uint32_t j = 0; j < textures_.size(); ++j) {
			if (std::strcmp(textures_[j].path.c_str(), str.C_Str()) == 0) {
		...
		if (!skip) {
			MeshTexture texture;
			texture.id = TextureFromFile(std::string(str.C_Str()), directory_);
